...
public class ListEnlaceDoble {

    NodListaDoble inicio; // Crea el puntero que apunta al inicio de la lista
    NodListaDoble fin; // Crea el puntero que apunta al fin de la lista

    public ListEnlaceDoble() {
        this.inicio = null;
        this.fin = null;
    }
    ...
    public void InsertaInicio(int dato) {
        NodListaDoble n = new NodListaDoble(dato); // Crea el nodo apuntado por el puntero n
        if (inicio == null) {
        ...
        if (pos > CuentaElementos()) { // Si la posición es mayor que la cantidad de elementos retorna mensaje
            ...
            if (pos == 0) {
                ...
                if (recorre.getSiguiente() == null) {
                    ...
                    NodListaDoble aux = recorre.getSiguiente();
    ...
    public void EliminaElemento(int pos) {
        ...
        if (pos == 0) {
            ...
            NodListaDoble nodoAEliminar = recorre.getSiguiente();
    ...
    public void EditaElemento(int pos, int newDato) {
        if (pos < 0 || pos >= CuentaElementos()) {
            System.out.println("La posición no existe en la lista");
            return;
        }
